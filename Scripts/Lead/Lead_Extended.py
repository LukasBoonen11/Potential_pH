...
dG_Chloride_Species = np.array((-131.2, -164.8, -426.3, -314.1, -391.2))          # Thermodynamic data - (0) Cl-, (1) PbCl+, (2) PbCl3-, (3) PbCl2, (4) Pb(OH)Cl

dG_Chloride_Species[0] = (dG_Chloride_Species[0] + (C[1] * T * np.log(Cl) * 1E-03))          # Concentration (activity) adjustment Cl-

dG_Chloride_Species[1] = (dG_Chloride_Species[1] + (C[1] * T * np.log(N) * 1E-03))          # Concentration (activity) adjustment PbCl+

dG_Chloride_Species[2] = (dG_Chloride_Species[2] + (C[1] * T * np.log(N) * 1E-03))          # Concentration (activity) adjustment PbCl3-


dG_Sulfate_Species = np.array((-744.5, -755.9, -813.1))          # Thermodynamic data - (0) SO42-, (1) HSO4-, (2) PbSO4
...
dG_Carbonate_Species = np.array((-527.8, -586.8, -625.5))          # Thermodynamic data - (0) CO32-, (1) HCO3-, (2) PbCO3
...
dG_Reaction_11_Grid = ((dG_Cl_Grid_1 + (2 * dG_Electron_Grid)) - (dG_Pb_Grid_0 + dG_Cl_Grid_0))

### Reaction 12 - PbCl3-

dG_Reaction_12_Grid = ((dG_Cl_Grid_2 + (2 * dG_Electron_Grid)) - (dG_Pb_Grid_0 + (3 * dG_Cl_Grid_0)))

### Reaction 13 - PbCl2

dG_Reaction_13_Grid = ((dG_Cl_Grid_3 + (2 * dG_Electron_Grid)) - (dG_Pb_Grid_0 + (2 * dG_Cl_Grid_0)))

### Reaction 14 - Pb(OH)Cl

dG_Reaction_14_Grid = ((dG_Cl_Grid_4 + dG_Proton_Grid + (2 * dG_Electron_Grid)) - (dG_Pb_Grid_0 + dG_H2O_Grid + dG_Cl_Grid_0))

### Reaction 15 - PbSO4

dG_Reaction_15_Grid = ((dG_SO4_Grid_2 + (2 * dG_Electron_Grid)) - (dG_Pb_Grid_0 + dG_SO4_Grid_0))

### Reaction 16 - PbCO3

dG_Reaction_16_Grid = ((dG_CO3_Grid_2 + (2 * dG_Electron_Grid)) - (dG_Pb_Grid_0 + dG_CO3_Grid_0))
...
np.savetxt(r'C:\\Users\\lukas.boonen\\OneDrive - Proviron Holding nv\\Desktop\\Research Papers\\Paper 1\\E_pH\\E_pH_Pb_V.csv', dG_Pourbaix_Diagram, delimiter = ';', comments='')
...
def Plot_Pourbaix_Diagram():
    ...
    plt.text(2.50, 0.25, r'PbCO$_{3 (s)}$', fontsize = 10)
    plt.text(8.50, 1.25, r'PbO$_{2 (s)}$', fontsize = 10)
    plt.text(9.35, -0.15, r'Pb(OH)$_{2 (s)}$', fontsize = 10)
...
Export_Plot(fig_Pourbaix_Diagram, r'C:\\Users\\lukas.boonen\\OneDrive - Proviron Holding nv\\Desktop\\Research Papers\\Paper 1\\E_pH\\E_pH_Pb_V', 1200)
