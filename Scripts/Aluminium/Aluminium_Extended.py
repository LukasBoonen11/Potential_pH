...
dG_Metal_Species[1] = (dG_Metal_Species[1] + (C[1] * T * np.log(M) * 1E-03))          # Concentration (activity) adjustment Al3+

dG_Metal_Species[2] = (dG_Metal_Species[2] + (C[1] * T * np.log(M) * 1E-03))          # Concentration (activity) adjustment AlO2-

dG_Metal_Species[3] = (dG_Metal_Species[3] + (C[1] * T * np.log(M) * 1E-03))          # Concentration (activity) adjustment AlOH2+

dG_Metal_Species[4] = (dG_Metal_Species[4] + (C[1] * T * np.log(M) * 1E-03))          # Concentration (activity) adjustment Al(OH)4-
...
np.savetxt(r'C:\\Users\\lukas.boonen\\OneDrive - Proviron Holding nv\\Desktop\\Research Papers\\Paper 1\\E_pH\\E_pH_Al_V.csv', dG_Pourbaix_Diagram, delimiter = ';', comments='')
...
def Plot_Pourbaix_Diagram():
    ...
    ax1.plot(np.repeat(6.4, 100), np.linspace(-3, 3, 100), '--', color = '#7E7E7E', alpha = 0.25)
    ax1.plot(np.repeat(10.4, 100), np.linspace(-3, 3, 100), '--', color = '#7E7E7E', alpha = 0.25)
    ...
    plt.text(5.45, -2.90, r'CO$_{2}$', fontsize = 6)
    plt.text(6.55, -2.90, r'HCO$_{3}^{-}$', fontsize = 6)

    plt.text(9.05, -2.90, r'HCO$_{3}^{-}$', fontsize = 6)
    plt.text(10.50, -2.90, r'CO$_{3}^{2-}$', fontsize = 6)
    ...
    plt.text(2.00, -2.50, r'Al$_{(s)}$', fontsize = 10)
    plt.text(0.00, -1.00, r'Al$_{(aq)}^{3+}$', fontsize = 10)
    plt.text(4.425, 0.25, r'Al$_{2}$O$_{3}$.3H$_{2}$O$_{(s)}$', fontsize = 10)
    plt.text(11.00, 0.00, r'Al(OH)$_{4 (aq)}^{-}$', fontsize = 10)
...
Export_Plot(fig_Pourbaix_Diagram, r'C:\\Users\\lukas.boonen\\OneDrive - Proviron Holding nv\\Desktop\\Research Papers\\Paper 1\\E_pH\\E_pH_Al_V', 1200)
